...
const A_details = ({ adata }: { adata: product }) => {
...
const handleCheckout = async () => {
  if (orderDetails.paymentMethod === "card") {
    ...
    const productData = {
      ...
      price_data: {
        currency: "usd",
        unit_amount: Math.round(adata.price * 100), // Convert to cents
      },
      quantity: 1,
      imageUrl: urlFor(adata.imageUrl).url(),
    };
          ...
          <div className="p-8 space-y-6">
            ...
            <div className="flex justify-center md:justify-start mt-8">
              <button
                ...
                className={`w-full md:w-auto bg-cyan-600 text-white px-6 py-4 rounded-lg font-semibold text-lg hover:bg-cyan-700 transition-colors ${adata.available
                  ? "bg-cyan-500 text-white hover:bg-cyan-600"
                  : "bg-red-600 text-white cursor-not-allowed hover:bg-red-600"
                  }`}
                onClick={() => setShowOrderForm(true)}
              >
                {adata.available ? "Buy Now" : "Out of Stock"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-8 max-w-md w-full text-center">
            ...
            <div className="text-left">
              ...
              <p><strong>Product:</strong> {adata.title}</p>
              <p><strong>Total:</strong> ${adata.price}</p>
            </div>
            <button
